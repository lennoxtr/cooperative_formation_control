...
def main():
    ...
    parser = argparse.ArgumentParser(description='Spawn multiple robots into Gazebo')
    ...
    root = tree.getroot()
    turtlebot3_imu_plugin = None
    turtlebot3_diff_drive_plugin = None 
    turtlebot3_laserscan_plugin = None
    turtlebot3_joint_state_plugin = None
    for plugin in root.iter('plugin'):
        if 'turtlebot3_diff_drive' in plugin.attrib.values():
            turtlebot3_diff_drive_plugin = plugin
        elif 'turtlebot3_imu' in plugin.attrib.values():
            turtlebot3_imu_plugin = plugin
        elif 'turtlebot3_laserscan' in plugin.attrib.values():
            turtlebot3_laserscan_plugin = plugin
        elif 'turtlebot3_joint_state' in plugin.attrib.values():
            turtlebot3_joint_state_plugin = plugin


    # We change the namespace to the robots corresponding one
    tag_diff_drive_ros_params = turtlebot3_diff_drive_plugin.find('ros')
    ...
    if turtlebot3_imu_plugin is not None:
        tag_imu_ros_params = turtlebot3_imu_plugin.find('ros')
        tag_imu_ns = ET.SubElement(tag_imu_ros_params, 'namespace')
        tag_imu_ns.text = '/' + args.robot_namespace + '/imu'
    else:
        print("ERROR>>>>>>>>>>>>>>>>>>>>> IMU NOT FOUND")

    if turtlebot3_laserscan_plugin is not None:
        tag_laserscan_ros_params = turtlebot3_laserscan_plugin.find('ros')
        tag_laserscan_ns = ET.SubElement(tag_laserscan_ros_params, 'namespace')
        tag_laserscan_ns.text = '/' + args.robot_namespace + '/laserscan'
    else:
        print("ERROR>>>>>>>>>>>>>>>>>>>>> LASERSCAN NOT FOUND")
    
    if turtlebot3_joint_state_plugin is not None:
        tag_states_ros_params = turtlebot3_joint_state_plugin.find('ros')
        tag_state_ns = ET.SubElement(tag_states_ros_params, 'namespace')
        tag_state_ns.text = '/' + args.robot_namespace + '/joint_states'
    else:
